...
### PrestaShop で BTCPay を使うには？

- [PrestaShop 用 BTCPay プラグインの利用](../PrestaShop.md)

### BTCPay には Shopify プラグインがありますか？
...
### 新しい BTCPay for WooCommerce V2 プラグインへアップグレードするには？
...
### エラー: 代替注文番号方式を使う場合は `class-wc-gateway-btcpay.php` で検索フィルターを適用してください
...
### 追加トークンサポート / Separate Payment Gateways の設定方法

:::warning 警告
このガイドは現在廃止された[レガシープラグイン](https://wordpress.org/plugins/btcpay-for-woocommerce/)向けであり、最新の[プラグイン V2](https://wordpress.org/plugins/btcpay-greenfield-for-woocommerce/)には適用されません。V2 プラグインではこの機能は現在「**Separate Payment Gateways**」と呼ばれます。以下のユースケースは引き続き有効ですが、[追加トークンの設定](#setup-your-additional-tokens) の手順に従う必要は**ありません**。代わりに、機能を有効化してサポートされるトークンを BTCPay Server インスタンスから自動取得する[設定項目](../WooCommerce.md#41-global-settings)が利用できます。
:::
...
- プロモーショントークンを使うには、BTCPay Server に [Liquid Assets プラグイン](https://github.com/btcpayserver/btcpayserver-plugins) をインストールしておく必要があります
...
前述の Liquid Assets プラグインの導入により、**プロモーショントークン**も受け付けられるようになります。これは商品やギフトの引き換えに使えるクーポンやバウチャーのようなものです。特徴として小数を持たず、商品数量 1 つあたり常に 1 トークンで支払う必要があります。
...
<a id="setup-your-additional-tokens"></a>
##### 追加トークンを設定する

<a id="setting-additional-token-configuration"></a>
###### 設定: 追加トークン構成（Additional token configuration）

BTCPay の支払い方法設定には、4 列の特定 CSV 形式でトークン設定を入力できる新しい設定 **「Additional token configuration」** があります。

1. **トークンシンボル（token symbol）**:
...
4. **トークンアイコン（任意 / token icon (optional)）**:
...
###### 設定: 追加トークン - 支払いトークンの強制（Additional tokens: Enforce payment tokens）

BTCPay Server のデフォルト支払い方法（Bitcoin）は、設定済みの通貨・アセット・アルトコイン・トークンを**制限しません**。つまり、デフォルトの支払い方法「Bitcoin」を有効化している場合、ユーザーは BTCPay Server の支払いページで、設定済みの通貨・アセット・アルトコイン・トークン（為替レートがあるもの）をすべて選択できます。これを望まない場合は、利用可能な支払いオプションを制限できます。このチェックボックスを選択すると、[設定: Additional token configuration](#setting-additional-token-configuration) に列挙した type が "payment" の通貨・アセット・アルトコイン・トークンのみに制限されます。

#### 追加トークンサポート機能を使った WooCommerce の一般的なユースケース

##### ユースケース 1: 商品を地域/配送ゾーンで制限する
...
2. 配送ゾーン / 配送方法の設定で、その配送クラスの料金を 0 にし、かつ「cost」を空または 0 に設定してください。また「no shipping class cost」は通常料金に設定します（以下はフラットレートの例）。
...
##### ユースケース 6: 商品の購入数をチェックアウトごとに 1 点に制限する
