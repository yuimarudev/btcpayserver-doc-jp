...
このドキュメントでは、BTCPay Server のアプリに関するよくある質問を扱います。

[[toc]]

## BTCPay のアプリとは何ですか？

アプリは、BTCPay のユースケースを拡張するために使える機能です。詳細は [Apps ドキュメント](../Apps.md) を参照してください。

## 作成できるアプリの数に上限はありますか？

アプリはストア単位で追加されます。作成するには、事前にストアをセットアップしておく必要があります。1つのストアに割り当てられるアプリ数に上限はありません。

## BTCPay に Point of Sale 機能はありますか？

はい。 [POS アプリの作成ガイド](../WhatsNext.md#creating-the-pay-button) をご覧ください。

## 実店舗で BTCPay を使うにはどうすればよいですか？

Point of Sale (PoS) アプリを利用できます。BTCPay Server 内で PoS アプリを作成すると、公開 URL でアクセスできるようになり、作成した商品のチェックアウトボタンが表示されます。
実店舗で PoS を使う最も簡単な方法（現時点）は、BTCPay で PoS アプリを作成し、スマートフォン、タブレット、PC などの Web 端末に表示することです。

詳しくは [モバイル端末での PoS アプリ利用ガイド](https://blog.btcpayserver.org/bitcoin-pos/) を参照してください。なお、セクション 2.3 の「Connecting a Wallet」は [ウォレットセクション](../WalletSetup.md) でより詳しく説明されています。

## BTCPay の Point of Sale アプリの見た目をカスタマイズする方法

Point of Sale アプリの見た目は簡単にカスタマイズできます。テーマ変更方法は [このガイド](../Development/Theme.md) を参照してください。
...
- [Point of Sale アプリを作成](../WhatsNext.md#creating-the-point-of-sale-app)
...
BTCPay Server のアプリには、サーバー本体のドメインとは異なるドメイン名を設定できます。たとえば、BTCPay Server が `mybtcpayserver.com` で動作していて、PoS アプリを `mybtcpayserver.com/apps/pos/abc123` ではなく `mybtcpaypos.com` に表示したい場合を考えます。
...
ドメイン名を入力し、ドロップダウンから事前に作成したアプリを選択して `save` をクリックすると、そのドメインにアプリをマッピングできます。
...
何らかの理由でアプリを BTCPay Server ホームページと同じドメインに置きたい場合は、ルートに表示する設定を選べます。この場合、ドメインはすでに正しく向いているため DNS 設定は不要です。BTCPay Server のルートドメインでアプリを使うと、ログインやその他のページに手動でアクセスする必要があります。最も簡単なのは、ルートドメインに `/apps` や `/stores` などのページルートを付ける方法です（例: `mybtcpayserver.com/apps`）。これによりルート表示アプリへの移動は簡単になりますが、他ページ（ログインなど）への移動は利用者にとって難しくなる可能性があります。

## 支払い後に別サイトへリダイレクトするには？

Point of Sale アプリでは、請求書が支払済みになった後に任意の URL へ顧客をリダイレクトできます。`Apps > Settings` でリダイレクト機能を変更してください。

![Point of Sale リダイレクト設定](../img/point-of-sale/AppRedirect.png)

PoS 設定における支払済み請求書のリダイレクトオプションは次のとおりです。

- **No（いいえ）** - _リダイレクト URL なし_
  - 請求書に、ユーザーを PoS アプリに戻すための案内が表示されます（デフォルト設定）。
- **No（いいえ）** - _リダイレクト URL あり_
  - 請求書に、指定した App Redirect URL に戻るための案内が表示されます。
- **Yes（はい）** - _リダイレクト URL なし_
  - 支払済み請求書が自動的に PoS アプリにリダイレクトされます。
- **Yes（はい）** - _リダイレクト URL あり_
  - 支払済み請求書が指定した App Redirect URL に自動リダイレクトされます。
- **Use Store Settings（ストア設定を使用）**
  - [ストアレベル](../FAQ/Stores.md#how-to-redirect-store-invoices-after-payment) で、PoS アプリへの自動リダイレクトを有効化/無効化します。

補足:

1. Redirect URL は App Settings（リダイレクトオプションの上）で設定してください。
2. 期限切れまたは一部支払い済みの [請求書](../Invoices.md#invoice-statuses) は、設定が有効でもリダイレクトされません。この機能は支払済み請求書専用です。
3. 代わりに API（例: Embedded PoS）経由で Redirect URL を指定することもできます。

## WooCommerce ストアを BTCPay の Crowdfund アプリに統合するには？

支援者にデジタルファイルや物理商品を提供したい場合、WooCommerce ストアを Crowdfunding アプリに埋め込めます。
...
1. WordPress サイト
2. [WooCommerce プラグイン](https://wordpress.org/plugins/woocommerce/)
3. [BTCPay for WooCommerce プラグイン](https://wordpress.org/plugins/btcpay-for-woocommerce/)
4. [Storefront テーマ](https://wordpress.org/themes/storefront/)（別テーマを使っている場合、CSS をテーマに合わせて調整する必要がある場合があります）
5. BTCPay Server
...
1. WooCommerce 用ストア
2. Crowdfunding アプリ用ストア

両方のストアが同期状態を保てるように、**同じ拡張公開鍵導出スキーム（extended public key derivation scheme）** を追加します。

#### 2. WordPress の CSS を変更する

最初のステップでは、WordPress ストアから冗長な要素を取り除き、Crowdfund アプリにスムーズに埋め込めるよう、シンプルに整える必要があります。
...
`Appearance>Editor>functions.php` に直接 PHP コードを追加すると、次回テーマ更新時に変更が消えます。カスタム関数プラグインを使うか、[子テーマを作成](https://docs.woocommerce.com/document/set-up-and-use-a-child-theme/) して、必ずその末尾にコードを配置してください。

#### 3. WordPress にスクリプトを追加する

[Header and Footer Scripts](https://wordpress.org/plugins/header-and-footer-scripts/) プラグインをインストールします。次のコードをヘッダーまたはフッターに追加してください。`Settings > Headers and Footers Script` でコードを貼り付けて保存します。
...
#### 4. Crowdfunding アプリを変更する

BTCPay で `Apps > Create New App > Crowdfunding` を開きます。

アプリの説明欄でコード表示を有効化し、次のコードを貼り付けて `<iframe src="http://yourdomain/shop/"></iframe>` を追加します。
`yourdomain` は WooCommerce ストアページの URL に置き換えてください。

![EmbedIframeCrowdfund](../img/CrowdfundCodeEmbed.png)

次に、Crowdfunding アプリの **Custom CSS Code** セクションに以下のコードを貼り付けます。
...
最後に、**Count all invoices created on the store as part of the crowdfunding goal（ストアで作成された全請求書をクラウドファンディング目標の集計に含める）** に必ずチェック（有効化）してください。
変更を保存し、アプリをプレビューします。
